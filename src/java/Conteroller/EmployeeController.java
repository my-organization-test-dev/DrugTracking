...
public class EmployeeController implements Serializable {
    ...
    public String create() {
        try {
             String no=this.ejbFacade.findAccountNO();
            getSelected().setEmployeeId("FMHACA01"+no+"ST");
            getFacade().create(current);
            Account acc=new Account();
            acc.setPassword(getSelected().getLastName()+"123@FMHACA");
            acc.setUsername("FMHACA01"+no+"ST");
            acc.setRole("Store");
            this.ejbFacade.addUser(acc);
